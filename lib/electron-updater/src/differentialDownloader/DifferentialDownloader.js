import { close, createWriteStream, open } from 'fs-extra';
import { URL } from 'url';
import { createHttpError, DigestTransform, configureRequestUrl, configureRequestOptions } from '../util-runtime/httpExecutor';
...
export class DifferentialDownloader {
  ...
  doDownload(oldBlockMap, newBlockMap) {
    // we don't check other metadata like compressionMethod - 
    // generic check that it is make sense to differentially update is suitable for it
  ...
  downloadFile(tasks) {
    const fdList = [];
    const closeFiles = () => Promise.all(fdList.map(openedFile => close(openedFile.descriptor)
      .catch(e => {
    ...
    await new Promise((resolve, reject) => {
      ...
      w = index => {
        ...
        const request = this.httpExecutor.createRequest(requestOptions, response => {
          // Electron net handles redirects automatically, 
          // our NodeJS test server doesn't use redirects - so, we don't check 3xx codes.
