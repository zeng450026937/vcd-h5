...
export function computeOperations(oldBlockMap, newBlockMap, logger) {
  ...
  if (oldEntry == null) {
    // new file (unrealistic case for now, because in any case both
    // blockmap contain the only file named as "file")
  ...
  const { checksumToOffset: checksumToOldOffset, checksumToOldSize } = buildChecksumMap(
    nameToOldBlocks.get(name), oldEntry.offset, logger
  );
...
function validateAndAdd(operation, operations, checksum, index) {
  if (isValidateOperationRange && operations.length !== 0) {
    ...
    if (lastOperation.kind === operation.kind 
      && operation.start < lastOperation.end 
      && operation.start > lastOperation.start) {
      const min = [ 
        lastOperation.start, lastOperation.end, operation.start, operation.end,
      ].reduce((p, v) => (p < v ? p : v));
