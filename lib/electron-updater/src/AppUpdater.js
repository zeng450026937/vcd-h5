...
import { CancellationToken, CancellationError } from './util-runtime/CancellationToken';
import { UUID } from './util-runtime/uuid';
import { asArray, newError } from './util-runtime';
...
import { DOWNLOAD_PROGRESS, UPDATE_DOWNLOADED, UpdaterSignal } from './UpdaterSignal';
...
export class AppUpdater extends EventEmitter {
  constructor(options, app) {
     ...
     * (if `quitAndInstall` was not called before).
     *
     ...
     * (e.g. `0.12.1-alpha.1`, here `alpha` is a prerelease component), otherwise `false`.
     *
     ...
     * (when a user from the beta channel wants to go back to the stable channel).
     *
     * Taken in account only if channel differs 
     * (pre-release version component in terms of semantic versioning).
     *
     * @default false
     */
    ...
    /**
     * For type safety you can use signals, 
     * e.g. `autoUpdater.signals.updateDownloaded(() => {})` 
   ...
   *
   * `allowDowngrade` will be automatically set to `true`. 
   * If this behavior is not suitable for you, simple set `allowDowngrade` explicitly after.
   */
  set channel(value) {
   ...
   * [GenericServerOptions](/configuration/publish#genericserveroptions) will be set with value as `url`.
  ...
  async isStagingMatch(updateInfo) {
    ...
    if (Number.isNaN(stagingPercentage)) {
  ...
  async getUpdateInfoAndProvider() {
    ...
    if (this.clientPromise == null) {
      this.clientPromise = this.configOnDisk.value
        .then(it => createClient(it, this, this.createProviderRuntimeOptions()));
    }
  ...
  onUpdateAvailable(updateInfo) {
    this._logger.info(`Found version ${updateInfo.version} (url: ${asArray(updateInfo.files).map(it => it.url || it.path).join(', ')})`);
  ...
  async getOrCreateDownloadHelper() {
    ...
    if (result == null) {
      ...
      try {
        dirName = (await this.configOnDisk.value).updaterCacheDirName;
      }
      catch (e) {
  ...
  async executeDownload(taskOptions) {
    ...
    const done = async isSaveCache => {
      await downloadedUpdateHelper.setDownloadedFile(
        updateFile, packageFile, updateInfo, fileInfo, updateFileName, isSaveCache
      );
